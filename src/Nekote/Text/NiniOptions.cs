...
public sealed record NiniOptions
{
    ...
    /// 
    /// - No section markers - files are flat key-value pairs only
    /// - Colon separator with no space: <c>Key:Value</c>
    /// - Case-sensitive keys and values (preserves exact casing in GUIDs, field names)
    /// - CRLF line endings (<c>\r\n</c>) for Windows compatibility
    /// - UTF-8 with BOM (ensures Windows applications correctly detect encoding)
    /// 
    ...
    /// Format characteristics:
    /// 
    /// - Section markers use brackets: <c>[Section]</c>
    /// - Equal sign separator: <c>Key=Value</c>
    /// - Case-insensitive keys and sections (matches Windows API behavior)
    /// - CRLF line endings (<c>\r\n</c>) following Windows INI tradition
    /// - UTF-8 without BOM (modern standard, ASCII-compatible for legacy tools)
    /// - Supports semicolon comments (<c>;</c>) in addition to <c>#</c> and <c>//</c>
    /// 
